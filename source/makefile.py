...
class Makefile:
	...
	def write(self, creator, name):
		...
		compiler_flags = self.project.settings.compiler_flags + ["-Wall", "-Wextra -Werror -std=c99 -pedantic"]
		...
		self.output_variable_list("ldflags", [linker_flags, link_string])
		...
		self.output_target_dependencies("$(executable)", ["$(objects)"], ["@echo Linking $@", "@$(cc) $(objects) $(ldflags)"])
